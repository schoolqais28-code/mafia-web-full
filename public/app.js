...
async function refreshMe(){me=(await api("/api/me")).user;$("#authBtn").classList.toggle("hidden",!!me);$("#logoutBtn").classList.toggle("hidden",!me);$("#adminBtn").classList.toggle("hidden",!me?.isAdmin);if(me&&!socket)connectSocket()}
async function refreshSite(){try{setAnnouncement((await api("/api/site")).announcement)}catch{}}
...
socket.on("voice:signal",handleSignal);socket.on("site:announcement",setAnnouncement);
